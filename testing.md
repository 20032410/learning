...
### 私有云存储
	目前市场上私人云存储产品大致可以分为四类：
	* 家庭电脑私人云存储，Syncbox、Polkast、Plex、LogMeln Pro、Spotdox
	* NAS私人云存储：Synology、Transporter、ZyxelNSA325、西部数据MyCloud
	* 智能路由私人云存储：Netgear、D-Link公司各自提供的WiFi路由器
	* WiFi移动私人云存储：在移动硬盘功能基础上增加电池供电和wifi接入，如西捷、kingston wi-drive、sandisk connect wireless mediadrive、从热死人 voyager air等。
	WitDisk
	RESTFUl风格
	Upnp
	DDns Server
...
数据包P↓［i］的延迟＝数据包P↓［i］接收时间－数据包P↓［i］发送时间
	...
	当前位置：首页 » 网络管理 » iperf iperf命令网络测试 iperf命令是一个网络性能测试工具。iperf可以测试TCP和UDP带宽质量。iperf可以测量最大TCP带宽，具有多种参数和UDP特性。iperf可以报告带宽，延迟抖动和数据包丢失。利用iperf这一特性，可以用来测试一些网络设备如路由器，防火墙，交换机等的性能。 iperf分为两种版本，Unix/Linux版和Windows版，Unix/Linux版更新比较快，版本最新。Windows版更新慢。Windows版的iperf叫jperf，或者xjperf。jperf是在iperf基础上开发了更好的UI和新的功能。
	...
	客户端单边：
	iperf -u -c 192.168.2.1 -b 100M -t 60
	客户端双边：
	iperf -u -c 192.168.2.1 -b 100M -t 60
	iperf -u -c 192.168.2.1 -d -b 100M -t 60这将使服务器端反向连接到客户端
